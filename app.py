...
def get_table_schema(table_name):
    """Fetch schema info (columns) of the specified table"""
    engine = get_database_connection()
    if engine is None:
        logger.error("Database connection failed in get_table_schema()")
        return None
    ...
    try:
        with engine.connect() as conn:
            ...
            logger.info(f"Fetched schema for table '{table_name}' with {len(schema_info)} columns")
            return schema_info
    except Exception as e:
...
def render_database_tab():
    ...
    df = get_table_data(table_name)  # Fetch table data

    # Get schema for the selected table
    schema_info = get_table_schema(table_name)  # Fetch table schema
    ...
    st.subheader(f"Table Overview: {table_name}")
    ...
    # # Quick stats for numeric and categorical columns
    ...
    #         st.write(f"**Top {col_name} values:**")
    #         top_numeric = df.nlargest(5, col_name)
    #
    #         # Dynamically include the numeric column + other relevant columns
    #         other_cols = [c for c in df.columns if c != col_name]
    #         display_cols = [col_name] + other_cols[:2]  # max 3 columns
    #         st.dataframe(top_numeric[display_cols], use_container_width=True)
    # else:
    ...
    #     with stats_col2:
    #         st.write(f"**Top categories in {col_name}:**")
    #         cat_counts = df[col_name].value_counts().head(5)
    #         for val, count in cat_counts.items():
    #             st.write(f"• {val}: {count}")
    # else:
    #     st.write("No categorical columns available for statistics.")
    ...
    if categorical_cols:
        search_col = st.selectbox("Select column to filter", categorical_cols)
        filtered_df = df.dropna(subset=[search_col])  # Keep rows with values in that column
    else:
        filtered_df = df
    ...
    if not filtered_df.empty:
        ...
        sort_by = st.selectbox("Sort by column", df.columns.tolist())
...
def initialize_model(_api_key):
    ...
    try:
        ...
        logger.info("LLM model (qwen/qwen-turbo) initialized")
        embed_model = HuggingFaceEmbedding(model_name="BAAI/bge-small-en-v1.5")
        logger.info("Embedding model (BAAI/bge-small-en-v1.5) initialized")
        ...
        logger.exception(f"Error initializing models: {str(e)}")
        return None, None
...
def handle_file_upload(uploaded_files):
    ...
    try:
        logger.info("Starting handle_file_upload")
        #1. Create a temporary directory if it doesn't exist yet
        if not hasattr(st.session_state, "temp_dir") or not os.path.exists(
            st.session_state.temp_dir
        ):
            st.session_state.temp_dir = tempfile.mkdtemp()
            logger.info(f"Created temp directory: {st.session_state.temp_dir}")
        else:
            logger.debug(f"Using existing temp directory: {st.session_state.temp_dir}")

        # 2. Track uploaded files
        if "uploaded_files" not in st.session_state:
            st.session_state.uploaded_files = []
            logger.debug("Initialized st.session_state.uploaded_files as empty list")

        # 3. Process each uploaded file
        file_paths = []
        for uploaded_file in uploaded_files:
            logger.info(f"Processing file selected by user: {uploaded_file.name}")

            # Save file to temporary location
            temp_file_path = os.path.join(st.session_state.temp_dir, uploaded_file.name)
            logger.warning(f"Overwriting existing file in temp dir: {temp_file_path}")
            ...
            st.session_state.uploaded_files.append({"name": uploaded_file.name, "path": temp_file_path})
            logger.info(f"Tracked uploaded file: {uploaded_file.name}")
            file_paths.append(temp_file_path)

        # 4. Set workflow-related flags this is to show in UI
        st.session_state.file_uploaded = True
        logger.debug("File upload flag set: user has successfully uploaded a file.")

        # Set first file as current for preview
        st.session_state.current_pdf = (file_paths[0] if file_paths else None)
        logger.debug(f"Current working PDF set to: {st.session_state.current_pdf}")

        # Mark workflow for update
        st.session_state.workflow_needs_update = True
        logger.debug("Workflow update required: triggering refresh of downstream steps")

        return file_paths
    except Exception as e:
        logger.exception(f"Error in handle_file_upload: {e}")
...
async def process_query(query, workflow):
    ...
    try:
        ...
        # query = f""" match the name of the city into our data base understand the name given and map it to one of the best
        # matching locations in our data , and follow the following query: '{query}' convert the following query to work in sql & return responses"""
        # logging.warn(query)
...
def main():
    ...
    with st.sidebar:
        ...
        # checking codex key, if it is different then this fucntion works
        if codex_key_input != st.session_state.codex_api_key:
            ...
            st.session_state.workflow_needs_update = True
            logger.info("User added codex api key")

        # Display Codex status
        if st.session_state.codex_api_key:
            st.success("Codex API Key: ✓")
            logger.info("Codex API Key Added Successfully")
            ...
            logger.info("Codex Api key is wrong or Enter correct key")
        ...
        if openrouter_key_input != st.session_state.openrouter_api_key:
            ...
            st.session_state.llm_initialized = False
            st.session_state.workflow_needs_update = True
            logger.debug("Session state updated: llm_initialized=False, workflow_needs_update=True")


        # Show status of Open Routerkey
        if st.session_state.openrouter_api_key:
            st.success("OpenRouter API Key: ✓")
            logger.info("Valid OpenRouter API key present")
        else:
        ...
        if not st.session_state.llm_initialized and st.session_state.openrouter_api_key:
            with st.spinner("Initializing models..."):
                logger.info("Attempting to initialize LLM and embedding models via OpenRouter...")
                ...
                if llm:
                    ...
                    logger.info("Models successfully initialized and stored in Settings")
                else:
                    st.error("Failed to initialize models")
                    logger.error("Model initialization failed using OpenRouter API key")
            ...
            logger.warning("Models not initialized due to missing OpenRouter API key")

        # File upload section in the sidebar
        logger.info("Initializing document upload section in the sidebar")
        st.header("Documents 📄")
        logger.debug("Displayed header: Documents")
        ...
        logger.debug(f"Files selected for upload: {uploaded_files}")

        if uploaded_files:
            logger.info(f"{len(uploaded_files)} file(s) selected by user")
            if st.button("Upload Documents"):
                logger.info("Upload button clicked")
                file_paths = handle_file_upload(uploaded_files)
                logger.debug(f"File paths after upload: {file_paths}")
                if file_paths:
                    st.success(f"{len(file_paths)} document(s) uploaded")
                    logger.info(f"{len(file_paths)} document(s) uploaded successfully")

                    # Display list of uploaded documents
                    st.write("Uploaded documents:")
                    logger.debug("Listing uploaded documents in session_state")
                    for i, file in enumerate(st.session_state.uploaded_files):
                        st.write(f"- {file['name']}")
                        logger.debug(f"Uploaded file listed: {file['name']}")

        # Document status indicator
        if st.session_state.file_uploaded:
            st.success("Documents: ✓")
            logger.info("Document status: Files uploaded")
        else:
            st.warning("Documents: ✗")
            logger.warning("Document status: No files uploaded yet")
    ...
    with col2:
        if st.button("🗄️ View Database"):
            ...
            logger.info("User clicked 'View Database' button. show_database set to True.")
        ...
        if st.session_state.file_uploaded:
            ...
            if file_key not in st.session_state.file_cache:
                with st.spinner("Processing documents..."):
                    logger.info(f"Processing uploaded documents for session {st.session_state.id}")
